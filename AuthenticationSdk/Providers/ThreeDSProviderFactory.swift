    ...
    public static func createProvider(preferredProvider: ProviderType? = nil) throws -> ThreeDSProvider {
        ...
        if availableProviders.isEmpty {
            throw AuthenticationError.noProviderAvailable(
                "No 3DS SDK provider found. Please ensure at least one supported 3DS SDK framework (Netcetera, Cardinal, Trident) is included in your project."
            )
        }
        ...
        if availableProviders.count == 1 {
            ...
            return tryCreateProvider(type: availableProviders[0])!
        } else {
